...
exports.createCustomer = async (req, res) => {
    try {
        ...
        sendNotification(email, `Welcome to Styles Laundry Service.\nThank you for registering with our laundry service.`);
...
exports.updateCustomer = async (req, res) => {
    try {
        ...
        const updatedCustomer = await Customer.findByIdAndUpdate(req.params.id, { name, phoneNumber, email, loyaltyPoints }, { new: true });
        if (!updatedCustomer) {
            return res.status(404).json({ message: 'Customer not found' });
        }
